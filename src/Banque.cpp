...
{
    bool rester;
    int adh_recherche_num_compte,tmp_verfication,j_verification;
    cout << "saisir le numero de compte de l'adherent recherché : ";
    do{
    tmp_verfication=0;
    j_verification=0;
    cin >> adh_recherche_num_compte;
    for (int i=0;i<this->Tab.size();i++)
    {
        if(this->Tab[i].get_Num_Compte()==adh_recherche_num_compte)
        {
            string type = this->Tab[i].get_type_compte();
            if(type=="livret A")
                {
                    //passage dans un switch case pour acceder au menu du livret A
                    cout<< "(Le compte auquel vous tentez d'acceder est un "<< type <<")"<<endl;
                    int choixlivreA;
                    cout << 
                        "================================\n"
                    ...
                    {      
                        cout<<" \nVeuillez resaisir un chiffre parmi les choix proposés : ";
                    }
            ...
            if(type=="compte courant")
                {
                    ...
                    int choixCourant;
                    cout << 
                    //passage dans un switch case pour acceder au menu du compte courant
                        "================================\n"
                        "1- La date d'ouverture\n"
                        "2- Le montent du decouvert autorise\n"
                        "3- Solde\n"
                        "4- Afficher l'historique -10 derniers operations-\n"
                        "5- toutes ses informations \n"
                        "0- Retour au menu precedent\n"
                        "================================\n"
                        "Choix : ";
            ...
            if(type=="compte bloque")
                {
                    ...
                    int choixBloque;
                    cout << 
                    //passage dans un switch case pour acceder au menu du compte bloqué
                        "================================\n"
                        "1- La date d'ouverture\n"
                        "2- Taux d interet\n"
                        "3- Solde\n"
            ...
            if(type=="epargne logement")
                {
                    ...
                    int choix_E_L;
                    cout << 
                    //passage dans un switch case pour acceder au menu du compte epargne et logement
                        "================================\n"
                        "1- La date d'ouverture\n"
                        "2- Taux d interet\n"
                        "3- Plafond\n"
...
void Banque::Afficher_banque_liste_adh_fichier()
{
    ...
    monfichier<<"Banque : ;"<<this->_nom<<" ; Adresse : ;"<<this->_adresse<<endl<<endl;
    monfichier<<"Prénom ; Nom ; Adresse ; e-Mail ; Numéro de téléphone ; Numéro de compte; Type de compte ; Solde du compte ; ";
    monfichier<<endl;
    for (int i=0;i<this->Tab.size();i++)
    {
...
{
    int adh_recherche_num_compte,tmp_verfication,numero_compte_local,a_recup=-1;
    ...
    for (int i=0;i<this->Tab.size();i++)
    {
        ...
        if(adh_recherche_num_compte==numero_compte_local)
        {
            a_recup=i;

            cout<<"Ce compte n"<<adh_recherche_num_compte<<" appartient a l adherent "<<this->Tab[a_recup].get_Nom_et_prenom()<<endl;
            Tab[i].Modifier_Adresse_Adh();
            tmp_verfication=0;
            break;break;
        }else{
                tmp_verfication=1;
            ...
            if (tmp_verfication==1&&i==this->Tab.size()-1)
            {
                cout<<"Ce numero de compte n'existe pas \nVeuillez en saisir un autre ";
                break;break;
            }
        }
    }while(tmp_verfication==1);
        
}

// permet d'ajouter un adherent préalablement remplis
void Banque::Ajouter(Adherent &A)
{
...
bool Banque::verifier_mdp(int a)
{
    ...
    void Banque::Ajout_Argent(int adh_rechrche_num_compte, double montent)
    {
        for (int i=0;i<this->Tab.size();i++){
        
        if(this->Tab[i].get_Num_Compte()==adh_rechrche_num_compte)
        {
            this->Tab[i].modifier_Solde(montent);
            }
    }
    }

    void Banque::retrait_Argent(int adh_rechrche_num_compte,double montent)
    {
        for (int i=0;i<this->Tab.size();i++){
